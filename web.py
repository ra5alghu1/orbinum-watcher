...
def build_incidents(rows, now, stale=STALE):
    ...
    def emit(kind, start, end, ending):
        ...
        out.append({'kind':kind, 'started':fmt(start),
                    'recovered':fmt(end) if ending=='recovered' else '—',
                    'duration':dur(end-start), 'duration_seconds':max(0,end-start),
